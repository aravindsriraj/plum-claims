...
- **API:** https://claims-api-968299856642.asia-south1.run.app (`GET /health`, `POST /claims`)
...
(images/PDF), and inspect the decision and full processing trace.
...
```bash
cd backend
.venv/bin/python -m pytest tests/        # 20 unit tests (rules, financial, fraud)
.venv/bin/python -m evals.run_evals      # 12 assignment test cases -> docs/EVAL_REPORT.md
```

Current status: **20/20 unit tests, 12/12 eval cases** (see
